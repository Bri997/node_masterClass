                    ...
                    _data.delete("checks", checkId, function(err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionError) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of the users checks. All checks may not have been deleted"
                          });
                        }
                      }
                    });
    ...
    _data.read("users", phone, function(err, userData) {
      if (!err) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          callback(400, {
            Error: "Password did not match the users stored password"
          });
        }
      } else {
        callback(400, { Error: "Could not find the user" });
  ...
  if (id && extend) {
    _data.read("tokens", id, function(err, tokenData) {
      if (!err && tokenData) {
        ...
        callback(400, { Error: "Specific token does not exist" });
      }
    });
  } else {
...
//Verify if a given token id is currently valid of the given users
        ...
        _data.read("users", userPhone, function(err, userData) {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              callback(400, {
                Error: "The user has already the max of checks"(
                  `${config.maxChecks}`
                )
              });
            }
...
//Checks -put
//Required data : id
//Optional data : protocol, url, method, success Codes, timeoutSeconds (one must be sent)
            ...
            _data.delete("checks", id, function(err) {
              if (!err) {
                _data.read("users", checkData.userPhone, function(
                  ...
                  if (!err && userData) {
                    // removed the deleted check from list of the check
                    let userChecks =
                      typeof userData.checks == "object" &&
                      userData.checks instanceof Array
                        ? userData.checks
